 ...
 * Port fitur existing (`selectRange shift+click` di DataList.js / tabel undangan).
 *
 * Pemakaian: panggil `toggle(index, id, shiftKey)` di onChange checkbox.
 * Klik biasa = toggle 1; shift+klik = pilih rentang dari anchor terakhir → index ini.
 */
import { useCallback, useMemo, useState } from 'react';

/**
 * Logika murni shift-select (dipisah supaya testable tanpa React).
 * Hitung Set terpilih baru setelah klik di `index`.
 */
